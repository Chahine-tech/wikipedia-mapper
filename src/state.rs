...
pub fn save_state(state: &CrawlState) -> Result<()> {
    let serialized = serde_json::to_string(state).context("Failed to serialize crawl state")?;
    let mut file = File::create("crawl_state.json").context("Failed to create crawl_state.json")?;
...
pub fn load_state() -> Result<CrawlState> {
    let file = File::open("crawl_state.json").context("Failed to open crawl_state.json")?;
    let state: CrawlState =
        serde_json::from_reader(file).context("Failed to deserialize crawl state")?;
    Ok(state)
}
